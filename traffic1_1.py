...
def local_data_import(folder):
    ...
    for day in range(365):
        ...
        df1, df2 = local_file_import(location, 2019, day)
        df_list.append(df1)
        df_list.append(df2)
    return df_list
...
def flow_speed_calculation (df_list, aggregation_time_period, mode):
    ...
    if mode == 0:
        ...
        for i in range(len(df_list)):
            ...
            print(flow_speed)
    flow_speed['density'] = (60/aggregation_time_period) * flow_speed.flow/flow_speed.mean_speed
    print(flow_speed)
    end_time = time.perf_counter()
...
df = pd.DataFrame() 
data_list = [[]]
path = 'trafficproject/data_19_10'
...
plt.scatter(df1.flow, df1.mean_speed)
#plt.show()
